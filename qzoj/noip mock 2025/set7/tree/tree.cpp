...
private:
    ...
    vector<vector<vector<lint>>> fx;
    vector<vector<int>> usz;
    vector<lint> h;
    auto dfs(int u,int f=-1)->vector<lint>&{
        if(!fx[u][f+1].empty()) return fx[u][f+1];
        usz[u][f+1]=1;
...
public:
    ...
    tree(int _n):gr(_n),usz(_n,vector<int>(_n+1)),fx(_n,vector<vector<lint>>(_n+1)){}
};
int main(){
