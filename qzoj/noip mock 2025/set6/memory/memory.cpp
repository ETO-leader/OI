...
int main(){
    ...
    cir(i,0,v){
        auto&uc=cnt[i];
        auto p=0;
        for(;p+4<n;p+=4) uc+=__builtin_popcount(a[p]+i)+__builtin_popcount(a[p+1]+i)+__builtin_popcount(a[p+2]+i)+__builtin_popcount(a[p+3]+i);
        for(;p<n;++p) uc+=__builtin_popcount(a[p]+i);
    }
    sparsetable st(cnt);
    cir(i,0,q){
        int l,r;inf>>l>>r;
