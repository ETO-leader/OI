...
int main(){
    ...
    vector<vector<int>> uc(k+7,vector<int>(k+7));
    vector<vector<lint>> f(k+4,vector<lint>(k+4));
    if(ty&1){
        const auto ax=a;
        a.emplace_back(1,1);
        a.emplace_back(n,m);
        ...
        ouf<<(MOD-f[k+1][k+1])%MOD<<'\n';
        a=ax;
    }
    if(ty&2){
        if([&]{cir(i,0,k) if(a[i]==make_pair(1,2)) return --k,false;return true;}()) a.emplace_back(1,2);
        if([&]{cir(i,0,k) if(a[i]==make_pair(2,1)) return --k,false;return true;}()) a.emplace_back(2,1);
        if([&]{cir(i,0,k) if(a[i]==make_pair(n-1,m)) return --k,false;return true;}()) a.emplace_back(n-1,m);
        if([&]{cir(i,0,k) if(a[i]==make_pair(n,m-1)) return --k,false;return true;}()) a.emplace_back(n,m-1);
        ...
        if(a.back().first==n-1&&a.back().second==m-1){
            a.pop_back();
            --k;
        }
        cir(i,0,k+4) cir(j,0,i){
            uc[i][j]=math.C(a[i].first-a[j].first+a[i].second-a[j].second,a[i].first-a[j].first);
        }
        f[0][1]=1;
        cir(i,1,k+3) cir(j,0,i) if(f[i][j]||f[j][i]){
            const auto stpx=uc[i+1][i];
            const auto stpy=uc[i+1][j];
