...
static array<uint64_t,maxx/64+3> tmp,utmp;
...
class calc{
    ...
    public:
        ...
        auto rotate(int up,int v){
            ...
            if(!lv){
                auto i=0,j=bv;
                for(;j+4<up-1;i+=4,j+=4){
                    tmp[j]=this->_M_w[i];
                    tmp[j+1]=this->_M_w[i+1];
                    tmp[j+2]=this->_M_w[i+2];
                    tmp[j+3]=this->_M_w[i+3];
                }
                ...
                for(;i<up;++i,++j) tmp[j]=this->_M_w[i];
                memcpy(this->_M_w,tmp.data(),sizeof(this->_M_w));
            }else{
                auto i=0,j=bv;
                for(;j+4<up-1;i+=4,j+=4){
                    tmp[j]=this->_M_w[i]<<lv;
                    tmp[j+1]=this->_M_w[i+1]<<lv;
                    tmp[j+2]=this->_M_w[i+2]<<lv;
                    tmp[j+3]=this->_M_w[i+3]<<lv;
                    utmp[j+1]=this->_M_w[i]>>rv;
                    utmp[j+2]=this->_M_w[i+1]>>rv;
                    utmp[j+3]=this->_M_w[i+2]>>rv;
                    utmp[j+4]=this->_M_w[i+3]>>rv;
                }
                for(;j<up-1;++i,++j){
                    tmp[j]=this->_M_w[i]<<lv;
                    utmp[j+1]=this->_M_w[i]>>rv;
        ...
        auto apply=[&](int u,int l,int r,const nodetag x){
            if(r-l+1>bcnt-1){
                if(x.cov>-1){
                    ...
                    if(~tg[u].cov) (tg[u].cov+=x.add)%=v;
                    else (tg[u].add+=x.add)%=v;
                    if(lasop[u]<0){
                ...
                if(x.add){
                    ...
                    fill(a.begin()+l,a.begin()+r+1,x.cov);
                    sg[u].set(x.cov);
                }
            }
        };
