...
private:
    bool neg,pos,any;
public:
    auto operator==(const sign_t y) const{
        if(any||y.any) return true;
        if(pos&&y.pos) return true;
        if(neg&&y.neg) return true;
        return false;
    }
    auto is_any() const{return any;}
    static auto neg_v(){return sign_t(true,false,false);}
    static auto pos_v(){return sign_t(false,true,false);}
    static auto any_v(){return sign_t(false,false,true);}
    sign_t(bool _x,bool _y,bool _z):neg(_x),pos(_y),any(_z){}
};
const auto sign(lf x){
    if(abs(x)<eps) return sign_t::any_v();
    return x>0?sign_t::pos_v():sign_t::neg_v();
}
auto&operator>>(istream&is,complex<lf>&u){
    lf x,y;is>>x>>y;
    u=complex<lf>(x,y);
    return is;
}
