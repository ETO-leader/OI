...
int main(){
    ...
    int n,m;cin>>n>>m;vector<int> p(n);
    dinic gr(n*2+m*2+7);
    const auto s=n*2+m*2,t=n*2+m*2+1;
    for(auto&i:p) cin>>i;
    cir(i,0,n) gr.link(i,i+n,p[i]),gr.link(i+n,t,p[i]);
    auto ans=0;
    cir(i,0,m){
        int u,v,c;cin>>u>>v>>c;--u;--v;
        ans+=c;
        gr.link(i+n*2,i+m+n*2,c);
        gr.link(i+m+n*2,u,_inf);
        gr.link(i+m+n*2,v,_inf);
        gr.link(s,i+n*2,_inf);
    }
