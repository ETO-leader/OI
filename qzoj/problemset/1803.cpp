...
public:
    auto value(){
        ...
        auto dfs=[&](auto __self,int u,int w,int lm){
            ...
            cir(uw,0,ul+1) if(!(uw==4||(w==6&&uw==2))) f[u][w][lm]+=__self(__self,u-1,uw,lm&&(uw==ul));
            return f[u][w][lm];
        };
        auto ans=0ll;
        cir(i,0,l-1) cir(w,1,10) if(w!=4) ans+=dfs(dfs,i-1,w,false);
        cir(w,1,lim[l-1]-'0') if(w!=4) ans+=dfs(dfs,l-2,w,false);
        if(lim[l-1]!='4') ans+=dfs(dfs,l-2,lim[l-1]-'0',true);
        return ans;
    }
