...
public:
    ...
    auto query(int l,int r){
        ...
        return array<lint,2>{min({x[0][0],x[0][1],x[1][0],x[1][1]}),min({x[1][0],x[1][1]})};
    }
...
int main(){
    ...
    int n,q;string s;cin>>n>>q>>s;vector<lint> w(n);
    for(auto&i:w) cin>>i;
    tree gr(n);
    cir(i,0,n-1){
        int u,v;cin>>u>>v;--u;--v;
    ...
    gr.init(w);
    cir(i,0,q){
        int a,x,b,y;cin>>a>>x>>b>>y;--a;--b;
        auto dlt=0ll;
        if(!x) gr.update(a,_infl);
        else gr.update(a,w[a]-_vinfl),dlt+=_vinfl;
        if(!y) gr.update(b,_infl);
        else gr.update(b,w[b]-_vinfl),dlt+=_vinfl;
        const auto val=gr.value()+dlt;
        cout<<(val<_infl?val:-1)<<'\n';
        gr.update(a,w[a]);
        gr.update(b,w[b]);
    }
    return 0;
