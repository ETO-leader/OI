...
pair<int,int> kth_ancestor(int x,int y,int k){
    if(!k) return make_pair(x,y);
    if(!x) return make_pair(x,y-k);
    if(!y) return make_pair(x-k,y);
    return lowbit(x)>lowbit(y)?
        kth_ancestor(x,y-min(lowbit(y),k),k-min(lowbit(y),k)):
        kth_ancestor(x-min(lowbit(x),k),y,k-min(lowbit(x),k));
}
bool is_ancestor(int x,int y,int qx,int qy){
    const auto dis=qx-x+qy-y;
    return dis>-1&&kth_ancestor(qx,qy,dis)==make_pair(x,y);
}
pair<int,int> lca(int x,int y,int qx,int qy){
...
int64_t distance(int x,int y,int ux,int uy){
