...
vector<int> solve(int n,int subtask){
    if(subtask==1){
        ...
        cir(i,0,n){
            ...
            cir(j,i+1,n){
                qry.emplace_back(j+1);
                qry.emplace_back(j+1);
            }
            qry.emplace_back(i+1);
        }
        const auto result=query(qry);
        ...
        [&]{
            ...
            cir(i,0,n){
                ++p;
                cir(j,i+1,n){
                    if(result[p]) es.emplace_back(i,j);
                    ++p;
                    ++p;
                }
                ++p;
        ...
        for(auto&[u,v]:es) gr[u].emplace_back(v),gr[v].emplace_back(u);
        auto p=gr[0][0];
        auto las=0;
        vector<int> stc{1};
        while(p){
        ...
        while([&]{
            ...
            // roundc=min(128,(int)(1.8e6)/invaild);
            auto cnt=0;
            ...
            for(auto&i:qry) ++i;
            const auto c=query(qry);
            vector<int> bp;
            vector<vector<int>> uqn;
            auto check=[&](int rl,int rr){
                qry.clear();
                cir(r,rl,rr){
                    vector<int> qn;
                    auto p=sp[r];
