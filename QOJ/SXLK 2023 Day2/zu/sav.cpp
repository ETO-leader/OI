...
private:
    ...
    auto accessable(int u,int v){
        if(res[v].win_usr==usr[u]) return true;
        return !fcnt[u];
    }
public:
    ...
    auto bfs(vector<int> rwin,vector<int> bwin,int qs){
        ...
        while(!qwq.empty()){
            ...
            if(u==qs) break;
            for(auto&v:gr[u]) if(update(v,u);(!vis[v])&&accessable(v,u)){
                vis[v]=true;qwq.emplace(v);
            }
        }
        return res[qs];
    }
    graph(int _n,vector<int> rusr,vector<int> busr):gr(_n),fcnt(_n),usr(_n,TBD),res(_n),vis(_n){
