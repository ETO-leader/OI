...
private:
    ...
    auto readarg(){
        _Ty res;cin>>res;ax.emplace_back(res);
        return res;
    }
public:
    auto init(){
        const auto n=readarg<int>(),q=readarg<int>();
        cir(i,0,n*3-2) readarg<int>();
        readarg<int>();
        array opin{2,1,3,1,1,2,3,2,2,2,2,1};
        cir(i,0,q){
            const auto op=readarg<int>();
            cir(c,0,opin[op]) readarg<int>();
        }
    }
    auto readargs(int&x){
        x=ax[cur].i32;++cur;
...
int main(){
    ...
    self_adjusting_top_tree satt(n);
    satt.chgroot(0);
    cir(i,0,n-1){
    ...
    int rt;inf.readargs(rt);--rt;
    satt.chgroot(rt);
    cir(i,0,q){
        int op;inf.readargs(op);
        if(!op){
            int u,w;inf.readargs(u,w);--u;
            satt.subfill(u,w);
        }else if(op==1){
            inf.readargs(rt);--rt;satt.chgroot(rt);
        }else if(op==2){
            int u,v,w;inf.readargs(u,v,w);--u;--v;
            satt.pathfill(u,v,w);
        }else if(op==3){
