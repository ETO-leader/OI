...
public:
    ...
    friend auto sqrt_s(poly a){
        assert(a[0]==1);
        auto res=exp(ln(a)*math.inv(2));
        res[0]=1;
        return res;
    }
};
...
public:
    static auto count(const int n){
        ...
        auto c=poly(nlen);
        c[0]=1;
        cir(i,1,nlen) c[i]=math.qpow(2,(lint)(i-1)*(i-2)/2)*math.ifact(i)%MOD;
        return ln(c)[n]*math.fact(n)%MOD;
    }
};
...
public:
    static auto count(const int n){
        ...
        auto f=poly(nlen);
        cir(i,0,nlen){
            f[i]=math.ifact(i)*math.inv(math.qpow(2,(lint)(i)*(i-1)/2))%MOD;
        }
        f=(f*f).sized(nlen);
        cir(i,0,nlen) (f[i]*=math.qpow(2,(lint)(i)*(i-1)/2)%MOD)%=MOD;
        f=sqrt_s(f);
...
int main(){
    ...
    const auto nlen=1<<(int)(ceil(log2(n+1)));
    math.init(nlen+7);
    cout<<euler_graph::count(n)<<'\n'<<bipartite_graph::count(n)<<'\n';
    return 0;
}
