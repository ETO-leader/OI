...
private:
    ...
    class compspecies{
    public:
        auto operator()(const pair<species&,environment*> a,const pair<species&,environment*> b) const{
            return a.first.value(a.second)<b.first.value(b.second);
        }
    };
    vector<species> sp;
    auto judge(){
...
public:
    auto evolve(){
        ...
        while(judge()<100.0){
            cerr<<"Generation = "<<++cnt<<'\n';
            vector<species> mx;
            cir(i,0,1080) mx.push_back(sp[i]);
            while(mx.size()<5800){
                auto u=rnd.randint(0,1499);
                auto v=rnd.randint(0,1499);
            ...
            sp.erase(unique(sp.begin(),sp.end(),[&](auto&a,auto&b){
                return a.value(this)>b.value(this);
            }),sp.end());
            ranges::sort(sp,[&](auto&a,auto&b){
                return a.value(this)>b.value(this);
